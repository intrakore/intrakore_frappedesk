...
frappe.ui.form.ControlLink = class ControlLink extends frappe.ui.form.ControlData {
	...
	setup_awesomeplete() {
		...
		this.$input.on("awesomplete-open", () => {
			...
			// Intrakore: also check against the nearest clipped grid container,
			// since .form-grid-container clips overflow-x independently of viewport width
			const gridContainer = this.$input && this.$input[0]
				? this.$input[0].closest(".form-grid-container")
				: null;
			const containerRight = gridContainer
				? gridContainer.getBoundingClientRect().right
				: viewportWidth;
			const rightBoundary = Math.min(viewportWidth, containerRight);
